...
// @access  Private
router.post('/', auth, upload.single('image'), validateCaseInput, async (req, res) => {
  try {
...
router.get('/map/locations', auth, async (req, res) => {
  try {
    ...
    const cases = await Case.find(query).select('location status priority animalType createdAt');
